...
void testApp::setup(){
    ...
    marioSprite.loadImage("images/mario/mariosprites.png");
    ...
    for (int k=0; k<nSounds; k++) {
        sounds[k].loadSound(DIR2.getPath(k));
    ...
    imgC.loadImage("images/sound/play.png");
    ...
    qcar->addTarget("Slits2.xml", "Slits2/Slits2.xml");
    //qcar->addTarget("Slits.xml", "Slits/Slits.xml");
...
void testApp::draw(){
    ...
    if(qcar->hasFoundMarker()) {
        ...
        if (mark == "xbee" || mark == "xbee2") {
            
            cout<<"xbee found"<<endl;
            //trigger=true;
            xbee=true;
            ofSetColor(ofColor::white);
            drawTextDefault(1);
...
void testApp::drawObjects(){
    ...
    if (trigger) {
        ...
        audimat.setLineHeight(34.0f);
//        audimat.drawString("Trigger",ofGetWidth()*5/7 ,ofGetHeight()*3/4+20);
          audimat.drawString("Play",ofGetWidth()*5/7 ,ofGetHeight()*3/4+20);
        ...
        audimat.drawString("Mario",ofGetWidth()*2/3-110, ofGetHeight()*3/4+20);
        ...
        ofSetColor(ofColor::white);
        //audimat.drawString("LED",ofGetWidth()/3+130, ofGetHeight()*3/4+20);
        audimat.drawString("Contra",ofGetWidth()/3+130, ofGetHeight()*3/4+20);
        ofPopMatrix();
        ...
        ofSetColor(ofColor::white);
        //audimat.drawString("Lamp",ofGetWidth()/3+180, ofGetHeight()*3/4+20);
        audimat.drawString("Pokemon",ofGetWidth()/3+160, ofGetHeight()*3/4+20);
        ofPopMatrix();
    }
...
//            ofLine(-70, images[currentImage].height/2, -50, images[currentImage].height/2+20);
        ...
        v1 = qcar->getMarkerCorner(OFX_QCAR_MARKER_CORNER_TOP_LEFT);
        v2 = qcar->getMarkerCorner(OFX_QCAR_MARKER_CORNER_TOP_RIGHT);
        v3 =qcar->getMarkerCorner(OFX_QCAR_MARKER_CORNER_BOTTOM_LEFT);
        v4 =qcar->getMarkerCorner(OFX_QCAR_MARKER_CORNER_BOTTOM_RIGHT);
        ofLine(v1.x, v1.y, posx, posy);
        ofLine(v2.x, v2.y, posx, posy);
        ofLine(v3.x, v3.y, posx, posy);
        ofLine(v4.x, v4.y, posx, posy);
        ...
        //audimat.setLineHeight(34.0f);
        //audimat.drawString("Drag image",ofGetWidth()/3+ofGetWidth()/4,ofGetHeight()*3/4 + 135);
        ...
        //if (monitorDisplay) {
            ofxOscMessage m3;
            m3.setAddress( "/monitor/display" );
            m3.addIntArg(currentImage);
            m3.addIntArg(currentSound);
            m3.addIntArg(flag);
            sender3.sendMessage( m3 );
       // }
...
//            ofLine(-70, imgC.height/2, -50, imgC.height/2+20);
            ...
            imgC.draw(0,0, imgC.width, imgC.height);
            ofPopMatrix();
            ...
            ofScale(0.5, 0.5);
            ofTranslate(posx-images[currentImage].width/2, posy-images[currentImage].height/2);
            imgC.draw(posx,posy, imgC.width,imgC.height);
            audimat.setLineHeight(17.0f);
            audimat.drawString(DIR2.getName(currentSound),posx, posy-images[currentImage].height/2 + 100);
...
//            audimat.drawString("Select",ofGetWidth()/3+ofGetWidth()/4,ofGetHeight()*3/4 + 135);
    ...
    if(!sound){
        ...
        //sounds[currentSound].setPosition(0);
    ...
    if (text) {
        ...
        //audimat.drawString(keyboard->getText()+" Received text:"+receivedText,mPos.x,mPos.y);
        audimat.drawString(keyboard->getText(),mPos.x,mPos.y);
        }
    ...
    if (xbee) {
        ...
        ofxOscMessage m4;
        m4.setAddress("/only/flag");
        m4.addIntArg(currentImage);
        m4.addIntArg(flag);
        sender2.sendMessage(m4);
        cout<<"Flag - "<<flag<<endl;
        ...
        ofFill();
        audimat.setLineHeight(34.0f);
        audimat.drawString("Connect",ofGetWidth()*5/7 ,ofGetHeight()*3/4+20);
        ...
        mainMenuElements[1].displayChildrenTrigger();
        ofSetColor(ofColor::white);
        audimat.drawString("Display",ofGetWidth()*2/3-110, ofGetHeight()*3/4+20);
        ...
        mainMenuElements[2].displayChildrenTrigger();
        ofSetColor(ofColor::white);
        audimat.drawString("Lamp",ofGetWidth()/3+120, ofGetHeight()*3/4+20);
        ofPopMatrix();
...
//        mainMenuElements[3].displayChildrenTrigger();
//        ofSetColor(ofColor::white);
//        audimat.drawString("Speaker",ofGetWidth()/3+200, ofGetHeight()*3/4+20);
//        ofPopMatrix();
        ...
        if (lampText) {
            ofLine((ofGetWidth()*2/3-2*ofGetWidth()/4 + ofGetWidth()*2/3-ofGetWidth()/4)/2, (ofGetHeight()*3/4-20 + ofGetHeight()*3/4+40)/2, posx, posy);
            
            if (posx >ofGetWidth()*2/3-ofGetWidth()/4 && posx<=ofGetWidth()*2/3 && posy>=ofGetHeight()*3/4-20 && posy<=ofGetHeight()*3/4+40) {
                setTextSize(34.0f);
                drawText("Connected", ofGetWidth()/3, ofGetHeight()/2);
                connected=true;
                flag=1;
                led=true;
            }
        ...
        if (monitorDisplayText) {
            ofLine((ofGetWidth()*2/3-ofGetWidth()/4 + posx<=ofGetWidth()*2/3)/2, (ofGetHeight()*3/4-20 + posy<=ofGetHeight()*3/4+40)/2, posx, posy);
        
            if (posx >ofGetWidth()*2/3-2*ofGetWidth()/4 && posx<=ofGetWidth()*2/3-ofGetWidth()/4 && posy>=ofGetHeight()*3/4-20 && posy <=ofGetHeight()*3/4+40) {
                setTextSize(34.0f);
                drawText("Connected", ofGetWidth()/3, ofGetHeight()/2);
                connected=true;
                flag=1;
...
void testApp::drawLeftArrow(){
    
    ofLine(10, ofGetHeight()/2+65, 80, ofGetHeight()/2+65);
    ofLine(10, ofGetHeight()/2+65, 20, ofGetHeight()/2 + 85);
    ofLine(10, ofGetHeight()/2+65, 20, ofGetHeight()/2 + 45);
    
}

void testApp::drawRightArrow(){
    
    ofLine(ofGetWidth()-10, ofGetHeight()/2+65, ofGetWidth()-80, ofGetHeight()/2+65);
    ofLine(ofGetWidth()- 10, ofGetHeight()/2+65, ofGetWidth()-20, ofGetHeight()/2 + 85);
    ofLine(ofGetWidth()- 10, ofGetHeight()/2+65,ofGetWidth()- 20, ofGetHeight()/2 + 45);
